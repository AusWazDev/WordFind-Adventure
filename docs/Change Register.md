...
| DEF-36 | All game modes defaulted to Standard / Random / Easy regardless of selection. Root cause: CR-36 switched BrowserRouter → HashRouter for Capacitor WebView compatibility. With HashRouter, URL params live in the hash fragment — `window.location.search` is always empty, so `mode`, `category`, and `level` in `Game.jsx` always fell back to defaults. Fix: replaced `window.location.search` with `useSearchParams()` from react-router-dom, which is router-aware. ESLint `no-restricted-syntax` rules added to ban `window.location.search`, `.hash`, and `.assign` in components — compiler now errors with a message pointing to the correct hook. | ✅ Fixed | `a30f2d3` | Apr 2026 |
| DEF-37 | Game grid left-justified in portrait on wide screens (tablets, PCs). Root cause: portrait board container has `width: 100%` but `boardSize` is constrained by height (`min(55dvh, 100vw)`), making the board narrower than its container on wide screens. No centering applied so the board defaulted left. Landscape layout already had `justifyContent: 'center'` — portrait was missing it. Fix: added `display: flex; justifyContent: center` to the portrait board area container. Narrow phones unaffected — board fills the container on narrow viewports. Confirmed fixed on Android tablet. | ✅ Fixed | `371ddd7` | Apr 2026 |
| DEF-38 | No audio on Android Capacitor (and potentially iOS). Root cause: `AudioContext` starts suspended on Android/iOS due to autoplay policy. `unlockAudio()` called `_audioCtx.resume()` without awaiting it, then audio was scheduled immediately — before the context had finished resuming. Fix: made `unlockAudio()` async and added `await ctx.resume()`. Updated all call sites (`WordList.jsx`, `Settings.jsx`, `Game.jsx`) to `await unlockAudio()` before scheduling playback. Confirmed working on Android tablet after device volume was verified. | ✅ Fixed | `371ddd7` | Apr 2026 |
| DEF-39 | Android launcher icon and splash screen showing default Capacitor placeholder (blue X icon) instead of SoundFind branding. Root cause: CR-36 scaffolded the Capacitor Android project but did not generate branded assets. Fix: ran `@capacitor/assets generate --android` using `public/icon.png` as the source — generated 74 assets across all mipmap densities (ldpi → xxxhdpi) including adaptive icons, round icons, and portrait/landscape splash screens for light and dark modes. Confirmed branded icon and splash on Android tablet. | ✅ Fixed | `371ddd7` | Apr 2026 |
