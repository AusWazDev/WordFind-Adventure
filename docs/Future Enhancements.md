...
|---|------|-------|-------|--------|
| FE-04 | Leaderboard | Global Leaderboard | Real-time global leaderboard showing top scores across all players. Requires a backend (e.g. Supabase or Firebase) to store and query scores — not possible with the current localStorage-only architecture. Scope: per-mode and per-category leaderboards, weekly/all-time views, username entry on first high score. The existing Leaderboard page (`src/pages/Leaderboard.jsx`) is a stub ready to be wired up. Deferred to Phase 5+ alongside Capacitor and real IAP. | Apr 2026 |
